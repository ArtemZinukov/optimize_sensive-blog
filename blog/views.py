...
def index(request):
    ...
    most_popular_posts = list(Post.objects.prefetch_related('author').annotate(
    ...
    most_fresh_posts = list(Post.objects.prefetch_related('author').annotate(
        num_comments=Count('comments', distinct=True)
    ).order_by('-published_at')[:5:-1])
